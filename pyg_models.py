...
class PYGGraphMixin:
    ...
    def encode_flat(self, x_id: torch.Tensor, x_missing: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        x = self.encoder(x_id, x_missing)
        batch_size, num_nodes, feat_dim = x.shape
        return x.reshape(batch_size * num_nodes, feat_dim), batch_vector(batch_size, num_nodes, x.device)
    ...
    def graph_inputs(
        ...
        x_flat, batch = self.encode_flat(x_id, x_missing)
        batch_size, num_nodes = x_id.shape
        base_edge_index, base_edge_weight = edge_tensors_from_adjacency(adj.to(x_flat.device))
...
class PYGGCNClassifier(nn.Module, PYGGraphMixin):
    ...
    def forward(self, x_id: torch.Tensor, x_missing: torch.Tensor, adj: torch.Tensor) -> torch.Tensor:
        x, edge_index, batch, edge_weight = self.graph_inputs(x_id, x_missing, adj, use_edge_weight=True)
...
class PYGGINClassifier(nn.Module, PYGGraphMixin):
    ...
    def forward(self, x_id: torch.Tensor, x_missing: torch.Tensor, adj: torch.Tensor) -> torch.Tensor:
        x, edge_index, batch, _ = self.graph_inputs(x_id, x_missing, adj, use_edge_weight=False)
        layer_outputs = []
        for conv in self.convs:
            x = conv(x, edge_index)
            x = F.relu(x)
...
class PYGGATClassifier(nn.Module, PYGGraphMixin):
    def __init__(
        ...
        super().__init__()
        self.encoder = NodeEncoder(cat_dims, proj_dim=proj_dim)
        self.gat1 = GATConv(self.encoder.out_dim, hidden_dim, heads=heads, dropout=dropout)
...
class PYGTGCNClassifier(nn.Module, PYGGraphMixin):
    ...
    def forward(self, x_id: torch.Tensor, x_missing: torch.Tensor, adj: torch.Tensor) -> torch.Tensor:
        ...
        base_edge_index, base_edge_weight = edge_tensors_from_adjacency(adj.to(x.device))
        edge_index, edge_weight = repeat_edge_tensors(
            base_edge_index,
            base_edge_weight,
        ...
        batch = batch_vector(batch_size * timesteps, num_nodes, x.device)
