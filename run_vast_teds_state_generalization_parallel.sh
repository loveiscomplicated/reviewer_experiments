...
RESULTS_ROOT="${RESULTS_ROOT:-results_state_generalization_vast}"
MERGED_DIR="${MERGED_DIR:-results_state_generalization_vast_merged}"
RCLONE_REMOTE="${RCLONE_REMOTE:-}"
RCLONE_DEST_DIR="${RCLONE_DEST_DIR:-TEDS_GNN_reviewer_state_generalization_results}"
DISCORD_BOT_NAME="${DISCORD_BOT_NAME:-TEDS GNN Bot}"
UPLOAD_RESULTS="${UPLOAD_RESULTS:-0}"

SCENARIOS=(${SCENARIOS:-scenario01 scenario02})
MODELS=(${MODELS:-gcn gin gat})
GRAPH_TYPES=(${GRAPH_TYPES:-statistical})
...
echo "[$(ts)] ===== run_vast_teds_state_generalization_parallel start ====="
notify "[START] TEDS state-generalization parallel run started. host=$(hostname 2>/dev/null || echo unknown)"
...
if [[ "$GPU_COUNT" -lt 1 ]]; then
  ...
  notify "[FAIL] TEDS state-generalization parallel run failed: no CUDA GPU detected."
  exit 1
fi
...
if [[ -n "${JOB_INDEX_RANGE:-}" || -n "${JOB_INDEXES:-}" ]]; then
  ...
  declare -a ALL_JOB_SCENARIOS=("${JOB_SCENARIOS[@]}")
  ...
  for idx in "${SELECTED_JOB_INDICES[@]}"; do
    JOB_SCENARIOS+=("${ALL_JOB_SCENARIOS[$idx]}")
...
notify "[START] TEDS state-generalization matrix ready. matrix_total_jobs=${MATRIX_TOTAL_JOBS} selected_jobs=${TOTAL_JOBS} job_index_spec=${JOB_INDEX_SPEC} detected_gpus=${GPU_COUNT}"
...
while [[ "${#ACTIVE_PIDS[@]}" -gt 0 ]]; do
  ...
  for i in "${!ACTIVE_PIDS[@]}"; do
    ...
    if [[ "$rc" -ne 0 ]]; then
      ...
      notify "[FAIL] TEDS state-generalization job failed. label=${label} gpu=${gpu} rc=${rc}"
      FAIL_RC="$rc"
      FAIL_LABEL="$label"
    else
      echo "[$(ts)] job complete label=$label gpu=$gpu"
      notify "[SUCCESS] TEDS state-generalization job completed. label=${label} gpu=${gpu}"
    fi
...
if [[ "$FAIL_RC" -ne 0 ]]; then
  ...
  notify "[FAIL] TEDS state-generalization parallel run stopping without aggregation. failed_job=${FAIL_LABEL} rc=${FAIL_RC}"
  exit "$FAIL_RC"
fi
...
if [[ "$UPLOAD_RESULTS" == "1" && -n "$RCLONE_REMOTE" && "$UPLOAD_RAW_RESULTS" == "1" ]]; then
  ...
  if rclone copy "$RESULTS_ROOT" "${RCLONE_REMOTE}:${RCLONE_DEST_DIR}/raw" \
    ...
    --stats 10s
  then
    notify "[UPLOAD_OK] TEDS state-generalization raw selected results uploaded. job_index_spec=${JOB_INDEX_SPEC} remote=${RCLONE_REMOTE}:${RCLONE_DEST_DIR}/raw"
  else
    UPLOAD_RC=$?
    notify "[UPLOAD_FAIL] TEDS state-generalization raw selected upload failed. job_index_spec=${JOB_INDEX_SPEC} rc=${UPLOAD_RC} results_root=${RESULTS_ROOT}"
    exit "$UPLOAD_RC"
  fi
...
set +e
python aggregate_state_generalization_results.py \
...
if [[ "$AGG_RC" -ne 0 ]]; then
  notify "[FAIL] TEDS state-generalization aggregation failed. rc=${AGG_RC} results_root=${RESULTS_ROOT}"
  exit "$AGG_RC"
fi
...
if [[ "$UPLOAD_RESULTS" == "1" && -n "$RCLONE_REMOTE" ]]; then
  ...
  if rclone copy "$MERGED_DIR" "${RCLONE_REMOTE}:${MERGED_REMOTE_DIR}" \
    ...
    --stats 10s
  then
    notify "[UPLOAD_OK] TEDS state-generalization merged results uploaded. job_index_spec=${JOB_INDEX_SPEC} remote=${RCLONE_REMOTE}:${MERGED_REMOTE_DIR}"
  else
    UPLOAD_RC=$?
    notify "[UPLOAD_FAIL] TEDS state-generalization merged upload failed. rc=${UPLOAD_RC} merged_dir=${MERGED_DIR}"
    exit "$UPLOAD_RC"
  fi
...
notify "[SUCCESS] TEDS state-generalization parallel run completed. total_jobs=${TOTAL_JOBS} merged_dir=${MERGED_DIR}"
